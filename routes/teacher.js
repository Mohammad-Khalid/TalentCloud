...
router.get("/",function(req,res){
	connection.query("select * from teacher_info t, account a where t.account_id=a.id and enabled='1' and a.user_type_uid=2",function(err,docs){
		if(err) throw err;
		else
        {
            console.log(docs);
...
router.get("/:id",function(req,res){
    connection.query("select * from teacher_info t, account ac,address a, organization o where t.account_id=ac.id and ac.address_uid=a.address_uid and ac.orgid=o.orgid and ac.id='"+req.params.id+"' and ac.user_type_uid=2 and enabled='1'",function(err,docs){
        if(err) throw err;
        else
        {
            //console.log(docs);
                                                            ...
                                                            genericFunc.insertIntoDB(acc_obj, 'account', function (err, state1) {
                                                                if (err) {
                                                                    ...
                                                                    var teach_obj = {
                                                                        ...
                                                                        reports_to : req.body.org_id,
                                                                        qualification:obj.qualification
                                                                    };
                                                                    genericFunc.insertIntoDB(teach_obj, 'teacher_info', function (err, state1) {
                                                                        if (err) {
                                                                            no_of_not_inserted_rec++;
                                                                            Status.push(err);
                                                                            callback();
...
router.put("/",function(req,res){
    var Status=[];
    console.log(JSON.parse(req.body.teach));
    var obj = JSON.parse(req.body.teach);
                                   ...
                                   if(req.files[0] !== undefined)
                                    ...
                                    genericFunc.updateIntoADB(acc_obj,obj.ID, 'account', function (err, state1) {
                                        if (err) {
                                            ...
                                            genericFunc.updateIntoSDB(teach_obj,obj.ID, 'teacher_info', function (err, state1) {
                                                if (err) {
                                                    Status.push(err);
                                                    console.log("teacher_info error!!!!!!!!!!!");
                                                     res.send({status:false,Status:Status});
